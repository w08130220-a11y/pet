...
function ProviderCard({ provider, onPress, isFavorite, onToggleFavorite }: {
  ...
  return (
    <div
      onClick={onPress}
      className="bg-surface rounded-xl border border-border p-4 mb-3 cursor-pointer hover:shadow-sm transition-shadow"
    >
      <div className="flex justify-between items-start">
        <div className="flex-1 mr-3">
          <div className="flex items-center mb-1">
            <span className="text-[17px] font-semibold text-foreground">{provider.name}</span>
            {provider.isVerified && (
              <span className="ml-1.5 bg-primary text-surface text-[10px] font-semibold rounded px-1.5 py-0.5">
                認證
              </span>
            )}
          </div>
      ...
      <div className="flex flex-wrap gap-1.5 mt-2.5">
        {provider.services.slice(0, 3).map((service) => (
          <span key={service.id} className="bg-background rounded-md px-2 py-1 text-xs text-muted">
            {service.name}
          </span>
        ))}
        {provider.services.length > 3 && (
          <span className="bg-background rounded-md px-2 py-1 text-xs text-muted">
            +{provider.services.length - 3}
          </span>
        )}
      </div>
    ...
    <div className="min-h-screen bg-background">
      <div className="pt-5 px-5">
        ...
        >
          ...
          <span className={`text-[13px] font-medium ${state.selectedCity ? 'text-foreground' : 'text-muted'}`}>
        ...
        {showCityFilter && (
          <div className="flex overflow-x-auto hide-scrollbar mb-3 gap-2">
            <button
              ...
              className={`shrink-0 px-3.5 py-1.5 rounded-full border text-[13px] ${
                !state.selectedCity
